...
from softgym.action_space.action_space import (
    Picker,
    PickerPickPlace,
    PickerQPG,
    PickAndPlace,
...
class ClothEnv(FlexEnv):
    ...
    def get_default_config(self):
        ...
        if self.action_mode in ["sawyer", "franka"]:
            cam_pos, cam_angle = np.array([0.0, 1.62576, 1.04091]), np.array(
                [0.0, -0.844739, 0]
            )
        else:
            cam_pos, cam_angle = np.array([-0.0, 0.82, 0.82]), np.array(
                [0, -45 / 180.0 * np.pi, 0.0]
            )
        config = {
    ...
    def _get_obs(self):
        ...
        if self.observation_mode == "point_cloud":
            particle_pos = (
                np.array(pyflex.get_positions()).reshape([-1, 4])[:, :3].flatten()
            )
    ...
    def set_scene(self, config, state=None):
        ...
        if self.version == 2:
            robot_params = [1.0] if self.action_mode in ["sawyer", "franka"] else []
